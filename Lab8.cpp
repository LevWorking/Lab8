...
int main()
{
   ...
   /* PhysicsRectangle right;
    ...
    Font fnt;
    if (!fnt.loadFromFile("liberation-sans.ttf")) {
    ...
    Time lastDuck(clock.getElapsedTime());
    Time currentDuck(lastDuck);
    ...
    while ((arrows > 0) || drawingArrow) {
        ...
        /*for (PhysicsShape& ducks : duck) {
            if ((PhysicsSprite&)ducks == &duck) {
                if((PhysicsSprite&)duck.getPosition()[1] >= window.getSize().x)
                {
                    duck.setPosition(duck.x, 0);
                }
            };
        }*/
        

        currentDuck = clock.getElapsedTime();
        Time deltaDuck = currentDuck - lastDuck;
        long duckMS = deltaDuck.asMilliseconds();

        if (duckMS > 800) {
            PhysicsSprite& duck = ducks.Create();
            duck.setTexture(redTex);
            
            duck.setSize(Vector2f(200, 200));
            int x = 0;
            Vector2f sz = duck.getSize();
            duck.setCenter(Vector2f(x, 20 + (sz.y / 2)));
            ...
            world.AddPhysicsBody(duck);
            duck.onCollision =
                [&redTex, &drawingArrow, &world, &arrow, &duck, &ducks, &score, &popSound]
                (PhysicsBodyCollisionResult result) {
                if (result.object2 == arrow) {
                    popSound.play();
                    drawingArrow = false;
    ...
    Clock EndGame;
    Time EndYet(EndGame.getElapsedTime());
    while (EndYet < sf::seconds(3)) {
        EndYet = EndGame.getElapsedTime();
    };
    exit(0);
}
